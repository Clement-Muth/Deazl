...
export const ShareModalBody = ({
  ...
  const emailLink = `mailto:?subject=${encodeURIComponent(`Shopping List: ${listName}`)}&body=${encodeURIComponent(`Check out my shopping list: ${shareLink}`)}`;
  const whatsappLink = `https://wa.me/?text=${encodeURIComponent(`Check out my shopping list "${listName}": ${shareLink}`)}`;
